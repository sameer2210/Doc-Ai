...
export async function createWorkingImageForCrop(image: WorkflowImage): Promise<WorkflowImage> {
  ...
  const workingImage = await manipulateImage(localUri, normalizedActions, shouldResize ? 0.92 : 1);
...
export function calculateCropViewportRect({
  ...
  const cropRect = roundCropRectangle({
    originX: clamp((guideLeft - renderedLeft) / effectiveScale, 0, imageWidth - visibleSourceWidth),
    originY: clamp((guideTop - renderedTop) / effectiveScale, 0, imageHeight - visibleSourceHeight),
